...
parsed_folder <- function(folder_path_list){
  ...
  for (i in file_path_list){
    ...
    for (j in i){
      ...
      df_info <- data.frame(JobName, WaitingTime, RunTime, TimeLimit, NumCPUs,
                            #NA,NA   ##modif
...
#main part of script
if(file.exists(opt$in_path) & file.exists(opt$out_path)) {  #TRUE ##modif
  ...
  actual_date_time <- strsplit(x= as.character(Sys.time()), split = " ")[[1]][1:2] %>%
                      paste(collapse ="T") %>% 
                      gsub(pattern = ":", replacement = ".")
  file_name <- paste(c(opt$name, actual_date_time), collapse ="_") #"name_test" ##modif
  ...
  pdf_file_name <- paste(c(opt$name,"plots", actual_date_time), collapse ="_") #"name_test" ##modif
...
```{r p_WaitingTime, echo=FALSE}
ggplotly(p_WaitingTime) %>% style(hoverlabel = label) %>% layout(font = font)
...
```{r p_RunTime, echo=FALSE}
ggplotly(p_RunTime) %>% style(hoverlabel = label) %>% layout(font = font)
...
ggplotly(p_Memory) %>% style(hoverlabel = label, marker.color = "#232F34") %>% layout(font = font) ##modif
