...
#   NumCPUs <<- strsplit(x = NumCPUs, split = c("=",":"))[[1]][6]
#   
#   #Memory_Efficiency                                                #OK
#   Memory_Efficiency <<- research_Element_PBS(FileInput_List, "Resources")
#   Memory_Efficiency <<- strsplit(x = Memory_Efficiency, split = c("=",","))[[1]][6]
#   Memory_Efficiency <<- strsplit(x = Memory_Efficiency, split = "kb" )[[1]][1]
#   
#   
#   #research JobId
...
#   checkjob_resp <<- system2("checkjob", args = JobId, stdout = TRUE)
#   
#   #calculation WaitingTime
#   WaitingTime <<- String_to_Date(research_Element_PBS(checkjob_resp, "Eligible:"))  #OK
# 
#   #Memory_Request
#   Memory_Request <<- as.numerical(NumCPUs) * 5                                      #OK
# 
#   # Memory_Efficiency <<- NA
#   # Memory_Request <<- NA
#   
...
Values_PBS <- function(FileInput_List){
  ...
  RunTime <<- hms(tail(strsplit(x = RunTime, split = "=")[[1]],1)) %>% 
                period_to_seconds()
  
  #research TimeLimit
  TimeLimit <<- research_Element_PBS(FileInput_List, "Limits")   #OK
  TimeLimit <<- hms(tail(strsplit(x = TimeLimit, split = "=")[[1]],1)) %>%
                  period_to_seconds()
