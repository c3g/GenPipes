...
parsed_folder <- function(folder_path_list){
  ...
  for (i in file_path_list){
    ...
    for (j in i){
      ...
      df_info <- data.frame(JobName, WaitingTime, RunTime, TimeLimit, NumCPUs, NA,NA   #modif
                            #Memory_Efficiency, Memory_Request
                            )
...
#output path not specified    #modif
# if(is.na(opt$out_path)){
#   opt$out_path <- opt$in_path
# }

#main part of script
if(TRUE) {  #file.exists(opt$in_path) & file.exists(opt$out_path) #modif
  
  # Options values                              #modif
  # job_output_path = opt$in_path
  # output_path = opt$out_path
  
  output_path = job_output_path                 #modif           
  ...
  actual_date_time <- strsplit(x= as.character(Sys.time()), split = " ")[[1]][1:2] %>%
                      paste(collapse ="T") %>% 
                      gsub(pattern = ":", replacement = ".")
  file_name <- paste(c("name_test", actual_date_time), collapse ="_") #opt$name #modif
  ...
  pdf_file_name <- paste(c("name_test","plots", actual_date_time), collapse ="_") #opt$name #modif
  ...
  #create pdf file from plots                                                     #modif
...
This plot represent the time spent in queue before steps get run. 

```{r eruptions, echo=FALSE, out.width="100px"}
# ui <- fluidPage(
...
#             dist=sqrt((hover$x-DF_plot$WaitingTime)^2)         #sqrt((hover$x-DF_plot$JobName)^2+(hover$y-DF_plot$WaitingTime)^2)
#             cat("hover_x \n")
#             cat(round(hover$x))
#             cat("\n")
#             cat("hover_y \n")
...
#             cat("\n")
#             cat("waitingTime \n")
#             cat(DF_plot$WaitingTime)
#             cat("\n")
#             cat("dedans ?")
#             cat(round(hover$x) %in% DF_plot$WaitingTime)
#             
#             if(min(dist) < 1)
#                 DF_plot$WaitingTime[which.min(dist)]
#         }
# 
# 
#     })
...
This plot represent the running time for each steps. 

```{r plot2, echo=FALSE}
ggplotly(p_WaitingTime)
ggplotly(p_RunTime)
...
<!--         if(!is.null(input$plot_hover)){ -->
<!--             hover=input$plot_hover -->
<!--             dist=sqrt((hover$x-DF_plot$RunTime)^2) -->
<!--             cat("hover_x \n") -->
<!--             cat(round(hover$x)) -->
<!--             cat("\n") -->
<!--             cat("hover_y \n") -->
...
<!--             cat("\n") -->
<!--             cat("dedans ?") -->
<!--             cat(round(hover$x) %in% DF_plot$RunTime) -->

<!--             if(min(dist) < 1) -->
<!--                 DF_plot$RunTime[which.min(dist)] -->
<!--         } -->
