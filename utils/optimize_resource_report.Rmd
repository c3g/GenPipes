...
#params$input <<- "/Users/mleguen/Documents/local/projet/optimize_resources_report"
#params$output <<- NA
#params$name <<- "Optimize_ressource_report"
#params$verbose <<- TRUE
...
parsed_folder <- function(folder_path_list){
  ...
  for (i in file_path_list){
    ...
    for (j in i){
      ...
      # #fill df_info with new information and rename columns
      ...
      # #type change 
      # df_info$RunTime <- as.character(df_info$RunTime)
      
      Info_df[nrow(Info_df)+1 ,] <- List_values

      
      #print("Info_df") #modif
      ##print(datatable(Info_df))
      #print("\n")
    ...
    # 
    # #change TimeLimit format
    # Info_df_temp$RunTime <- day_into_hours(Info_df_temp$RunTime)

    #print("CHANGE TIMELIMIT FORMAT DAY INTO HOUR") #modif
    #print(datatable(Info_df_temp))
    #print("\n")
	...
	#print("RUNTIME ET TYPE")
	#print(Info_df_temp$RunTime)
	#print(typeof(Info_df_temp$RunTime))
    ...
    # #print("INFO_DF AVANT BIND")
    # print(Info_df_temp) 
    #put Info_df_temp at the end of the main dataframe containing all information
    #Info_df <- rbind(Info_df, as.data.frame(Info_df_temp))
    # print("INFO_DF TOUTES LES MOYENNES TEMP")
    # print(Info_df_temp)

    # print("INFO_DF TOUTES LES MOYENNES")
    # print(Info_df)
 
  }
  #print("Info_df avant modif")
  #print(Info_df)
  
  #change dataframe name
  #names(Info_df) <- c("JobName", "WaitingTime", "RunTime", "TimeLimit", "NumCPUs", "Memory_Efficiency", "Memory_Request")
  
  #print("Info_df nom colonnes")
  #print(Info_df)
  ...
  #print("Info_df : RunTime et TimeLimit format")
  #print(Info_df)
  
  #Change WaitingTime format
  Info_df$WaitingTime <- ChangeWaitingTimeFormat(Info_df$WaitingTime)
  
  #print("Info_df WaitingTime format")
  #print(Info_df)
  
  #Specfific dataframe for ploting
  DF_plot <- Info_df
  
  #print("DF_plot après copie")
  #print(DF_plot)
  ...
  #print("Info_df keep moyenne pour chaque step")
  #print(Info_df)
...
Values_SLURM <- function(FileInput_List){
  ...
  out <- tryCatch(
    {
       ...
       return(c(Memory_Efficiency,Memory_Request))
    },error = function(cond){
  ...
  Memory_Efficiency <<- out[1]
  Memory_Request <<- out[2]
...
Values_PBS <- function(FileInput_List){
  ...
  #TimeEfficiency
  #TimeEfficiency <<- (RunTime / TimeLimit ) * 100                #OK
  ...
  ##### checkjob command for additional information #####                                   
    #checkjob_resp <<- system2("checkjob",
    #				 args = JobId,
    # 				 stdout = TRUE)
    #checkjob_resp <<- checkjob_file

    #calculation WaitingTime
    #Pos_cut <<-  grep("BecameEligible:",checkjob_resp)
    #checkjob_file_cut <<- checkjob_resp[c(Pos_cut+1:length(checkjob_resp))]
    #WaitingTime <<- hms::as_hms(research_Element_PBS(checkjob_file_cut, "Eligible:"))  #OK
  ...
  #                       stderr = TRUE
  #                      )
  # #print("LENGTH CHECKJOB_RESP")
  # #print(length(checkjob_resp))
  # #print("\n")
 
#   if(length(checkjob_resp) == 2){
# 	return(list(JobName,                #OK
#               #WaitingTime,           #OK
#               NA,
#               RunTime,                #OK
...
research_Element_PBS <- function(Input_file, Researched_element){
  ...
  out <- tryCatch(
    ...
    error = function(cond){
      print("The following element hasn't been found : ")
      print(Researched_element)
...
mean_value <- function(df){
  ...
  if (is.character(df)){
    ...
    df <- df %>%
      ...
      hms::hms()
      #seconds.to.hms()
    #print("DF APRES MOYENNE CHARACT")
    #print(df)
...
actual_date_time <- strsplit(x= as.character(Sys.time()), split = " ")[[1]][1:2] %>%
  paste(collapse ="T") %>% 
  gsub(pattern = ":", replacement = ".")
file_name <- paste(c(params$name, actual_date_time), collapse ="_") #temp_setup
...
```{r verbose, condition, eval=params$verbose, echo = FALSE}
#temp_setup
cat("*** \n ")
...
cat(paste0("**", nrow(Info_df),"** differents steps in total among all files." ,"  \n"))

#Absolute path if the one gived is relative
cat(paste0("Path used : **", opt$in_path,"**  \n"))

#scheduler choosed
cat(paste0("Scheduler choosed : **", params$scheduler,"**"))
...
This plot represent the time spend in queue for each step. Boxplot gives average, lower and longer time spent in queue.
...
if (params$scheduler == "SLURM"){
  ...
  if(!is.na(DF_plot$Memory_Efficiency)){  
    ...
    cat("**Memory plot plot hasn’t been displayed since no values have been found.**")  
  }
}else{
