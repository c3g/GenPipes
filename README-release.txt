...
git tag -s 3.2.0 -m 'Release 3.2.0'
git push -u origin --tags

# Recreate the CHANGELOG.md
bash ~/work/repo/dump_ChangeLog.sh > CHANGELOG.md
git commit -a -m "Version bump to 3.2.0"

# Create a release tarball archive
git archive --format=tar --prefix=genpipes-3.2.0/ 3.2.0 | gzip > ~/genpipes-3.2.0.tar.gz

# Upload this archive in
https://bitbucket.org/mugqic/genpipes/downloads

# Version bump the value. Until the next release, add '-beta' e.g. 3.2.1-beta
vim VERSION
git commit -m "Version bump to 3.2.1-beta" VERSION
git push
