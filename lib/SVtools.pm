...
sub runPairedDNAC {
  ...
  if (!$ro_job->isUp2Date()) {
    ...
    $command .= ' Rscript \${SVTOOLS_HOME}/Cancer/DNACRD.R';
    $command .= ' -f ' .$inputBins;
    $command .= ' -b ' .$window;
...
sub filterDNAC {
  ...
  my $ro_job = new Job();
  $ro_job->testInputOutputs([$inputDNACCalls], [$outputPrefix . '.filteredSV.txt'],$ro_job);

  if (!$ro_job->isUp2Date()) {
    my $command;
    $command .= LoadConfig::moduleLoad($rH_cfg, [['filterSV', 'moduleVersion.svtools']]) . ' &&';
    $command .= ' \${SVTOOLS_HOME}/filterOutDNACRD.sh';
    $command .= ' ' . $inputDNACCalls;
    $command .= ' ' . $outputPrefix . '.txt';
    $command .= ' ' . $sampleName;
    $command .= ' && ';
    $command .= filterDNACRDResults($rH_cfg, 'filterSV', $outputPrefix, $cnvProx);
    $command .= ' && ';
...
sub filterBrD {
  ...
  my $ro_job = new Job();
  $ro_job->testInputOutputs([$inputBrDCalls], [$outputPrefix . '.filteredSV.txt'],$ro_job);
...
sub filterPI {
  ...
  my $ro_job = new Job();
  $ro_job->testInputOutputs([$outputPrefix], [$outputPrefix . '.filteredSV.txt'],$ro_job);
...
###internal call only no object needed
...
#   if (!$ro_job->isUp2Date()) {
  ...
  $command .= ' \${SVTOOLS_HOME}/filterAnnotCNV.sh';
  $command .= ' ' . $outputPrefix . '.txt';
  $command .= ' ' . LoadConfig::getParam($rH_cfg, $stepIniPrefix, 'referenceMappabilityBed', 1, 'filepath');
  $command .= ' ' . LoadConfig::getParam($rH_cfg, $stepIniPrefix, 'referenceGeneCoordinates', 1, 'filepath');
  $command .= ' ' . LoadConfig::getParam($rH_cfg, $stepIniPrefix, 'referenceDGVCoordinates', 1, 'filepath');
...
#   if (!$ro_job->isUp2Date()) {
  my $command;
  $command .= LoadConfig::moduleLoad($rH_cfg, [['bedSV', 'moduleVersion.svtools'], ['filterSVC', 'moduleVersion.bedtools']]) . ' &&';
