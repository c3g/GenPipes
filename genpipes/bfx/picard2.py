...
def mark_duplicates(
    ...
    if not isinstance(inputs, list):
        inputs=[inputs]
    if global_conf.get(ini_section, 'module_picard').split("/")[2] < "2" and global_conf.get(ini_section, 'module_gatk').split("/")[2] < "4":
        return picard.mark_duplicates(inputs, output, metrics_file, remove_duplicates, create_index, ini_section=ini_section)
    elif global_conf.get(ini_section, 'module_gatk').split("/")[2] > "4":
...
java -Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram} -jar $PICARD_HOME/picard.jar MarkDuplicates \\
 ...
 MAX_RECORDS_IN_RAM={max_records_in_ram} {other_options}""".format(
            ...
            other_options=global_conf.get(ini_section, 'other_options',required = False) if global_conf.get(ini_section, 'other_options',required = False) else ""
            ),
            removable_files=[output, re.sub("\.([sb])am$", ".\\1ai", output), output + ".md5"]
        )
...
java -Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram} -jar $PICARD_HOME/picard.jar SamToFastq \\
  ...
  FASTQ={fastq}{second_end_fastq}""".format(
                ...
                other_options=global_conf.get('picard_sam_to_fastq', 'other_options'),
                input=input,
                fastq=fastq,
...
java -Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram} -jar $PICARD_HOME/picard.jar AddOrReplaceReadGroups \\
 ...
 {sequencing_center}""".format(
                ...
                sample=sample,
                sequencing_center=("RGCN=\"" + global_conf.get('picard_add_read_groups', 'sequencing_center') + "\"") if global_conf.get('picard_add_read_groups', 'sequencing_center', required=False) else ""
            )
        )
