...
python $STRELKA2_HOME/bin/configureStrelkaSomaticWorkflow.py \\
  ...
  --runDir {output}""".format(
            ...
            experiment_type=global_conf.get('strelka2_paired_somatic','experiment_type_option') if global_conf.get('strelka2_paired_somatic','experiment_type_option') else "",
...
python $STRELKA2_HOME/bin/configureStrelkaGermlineWorkflow.py \\
  ...
  --runDir {output}""".format(
            ...
            experiment_type=global_conf.get('strelka2_paired_germline','experiment_type_option') if global_conf.get('strelka2_paired_germline','experiment_type_option') else "",
            callRegions="\\\n  --callRegions " + callRegions if callRegions else "",
            output=output_dir
        )
    )
...
def run(
    ...
    ram = global_conf.get('strelka2_paired_somatic', 'ram')
