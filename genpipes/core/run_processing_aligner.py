...
class RunProcessingAligner(object):
    ...
    def get_rg_tag(readset, ini_section):
        return "'@RG" + \
               ...
               ("\tCN:" + global_conf.getget(ini_section, 'sequencing_center')
                if global_conf.getget(ini_section, 'sequencing_center', required=False) else "") + \
               "\tPL:Illumina" + \
               "'"
    ...
    def get_annotation_files(self):
        ...
        if os.path.isfile(ini_file):
            ...
            if genome_config.has_option(section, dbsnp_option_name) and\
                ...
                dbsnp_version = genome_global_conf.get(section, dbsnp_option_name)
                af_name = genome_global_conf.get(section, af_option_name)
                return [
...
class StarRunProcessingAligner(RunProcessingAligner):
    ...
    def get_reference_index(self):
        ...
        if os.path.isfile(ini_file):
            ...
            source = genome_global_conf.get("DEFAULT", "source")
            version = genome_global_conf.get("DEFAULT", "version")
