...
printf "{sample}\t{readset}\t${{minLen}}\t${{maxLen}}\t${{minFlashOverlap}}\t${{maxFlashOverlap}}\n" \\
  ...
  -j {filter_log}""".format(
    ...
    def merge_uchime_stats(self):
        ...
        for readset in self.readsets:
            flash_log = select_input_files([os.path.join("merge", readset.sample.name, readset.name + ".flash_pass2.log"),os.path.join("merge", readset.sample.name, readset.name + ".flash.log")])
