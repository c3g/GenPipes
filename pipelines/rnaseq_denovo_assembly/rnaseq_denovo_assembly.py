...
tmhmm --short \\
  ...
  > {tmhmm_output}""".format(
    ...
    def trinotate(self):
        ...
        if not config.param('trinotate', 'gene_column'):
            gene_id_column="#gene_id" 
        else:
            gene_id_column=config.param('trinotate', 'gene_column')
        if not config.param('trinotate', 'transcript_column'):
            transcript_id_column="transcript_id"
        else: 
            transcript_id_column=config.param('trinotate', 'transcript_column')
        return [concat_jobs([
...
$PYTHON_TOOLS/parseTrinotateOutput.py -r {trinotate_report} -o {trinotate_report_transcript_prefix} -i {transcript_id_column}
            ...
            gene_id_column = transcript_id_column,
...
do
  ...
  head -7 report/DiffExp/$contrast/${{contrast}}_Genes_DE_results_{swissprot_db}.tsv | cut -f-8 | sed '2i ---\t---\t---\t---\t---\t---\t---\t---' | sed 's/|/\\\\|/g' | sed 's/\t/|/g' >> {report_file}
    ...
    def differential_expression_goseq(self):
        ...
        Generates GO annotations for differential gene expression analysis, based on associated GOTERMS generated by trinotate.
        """
        ...
        for contrast in self.contrasts:
            ...
            job = differential_expression.goseq(
                ...
                os.path.join("trinotate", "trinotate_annotation_report.tsv.genes_go.tsv")
            )
