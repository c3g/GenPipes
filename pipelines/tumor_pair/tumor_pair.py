...
END`""".format(
                    ...
                    jobs.append(concat_jobs([
                        ...
                        pipe_jobs([
                            bcftools.concat(
                                ...
                                os.path.join(varscan_directory, tumor_pair.name + ".indel." + sequence['name'] + ".vcf.gz")], None),
                    ...
                    pipe_jobs([
                        bcftools.concat(
                            [os.path.join(varscan_directory, tumor_pair.name + ".snp.vcf.gz"),
                             os.path.join(varscan_directory, tumor_pair.name + ".indel.vcf.gz")], None),
                        Job([None], [output_vcf],
    ...
    def merge_mutect2(self):
        ...
        for tumor_pair in self.tumor_pairs.itervalues():
            ...
            if nb_jobs == 1:
                ...
                jobs.append(concat_jobs([
                    Job([input_vcf], [output_gz], command="ln -s -f " + os.path.abspath(input_vcf) + " " + os.path.abspath(output_gz), samples=[tumor_pair.normal, tumor_pair.tumor]),
    ...
    def strelka2_paired_somatic(self):
        ...
        for tumor_pair in self.tumor_pairs.itervalues():
            ...
            jobs.append(concat_jobs([
                Job(command="rm -rf " + strelka2_directory),
                ...
                pipe_jobs([
                    bcftools.concat(output_dep, None),
                    Job([None], [None],
                        command="sed 's/TUMOR/" + tumor_pair.tumor.name + "/g' | sed 's/NORMAL/" + tumor_pair.normal.name + "/g' | sed 's/Number=R/Number=./g' "),
                    htslib.bgzip_tabix(None, output_prefix + ".strelka2.vcf.gz"),
                ]),
                pipe_jobs([
                    vt.decompose_and_normalize_mnps(output_prefix + ".strelka2.vcf.gz", None),
                    htslib.bgzip_tabix(None, output_prefix + ".strelka2.vt.vcf.gz"),
                ]),
                tools.fix_genotypes_strelka(output_prefix + ".strelka2.vt.vcf.gz", output_prefix + ".strelka2.somatic.gt.vcf.gz", tumor_pair.normal.name, tumor_pair.tumor.name),
                bcftools.view(output_prefix + ".strelka2.somatic.gt.vcf.gz", output_prefix + ".strelka2.somatic.vt.vcf.gz", config.param('strelka2_paired_somatic', 'filter_options')),
            ...
            if use_bed:
                ...
                for bf in bed_file_list:
                    ...
                    jobs.append(concat_jobs([
                        mkdir_job,
                        pipe_jobs([
                            vardict.paired_java(input_normal[0], input_tumor[0], tumor_pair.name, None, bf),
                            vardict.testsomatic(None, None),
                            vardict.var2vcf(None, tumor_pair.normal.name, tumor_pair.tumor.name, None),
                            htslib.bgzip_tabix(None, os.path.abspath(output)),
                        ]),
                ...
                if nb_jobs == 1:
                    ...
                    jobs.append(concat_jobs([
                        ...
                        bedjob,
                        pipe_jobs([
                            vardict.paired_java(input_normal[0], input_tumor[0], tumor_pair.name, None, beds.pop()),
                            vardict.testsomatic(None, None),
                            vardict.var2vcf(None, tumor_pair.normal.name, tumor_pair.tumor.name, None),
                            htslib.bgzip_tabix(None, os.path.abspath(output)),
                        ]),
                    ...
                    for idx in range(nb_jobs):
                        ...
                        jobs.append(concat_jobs([
                            mkdir_job,
                            pipe_jobs([
                                vardict.paired_java(input_normal[0], input_tumor[0], tumor_pair.name, None, beds[idx]),
                                vardict.testsomatic(None, None),
                                vardict.var2vcf(None, tumor_pair.normal.name, tumor_pair.tumor.name, None),
    ...
    def merge_filter_paired_vardict(self):
        ...
        for tumor_pair in self.tumor_pairs.itervalues():
            ...
            if nb_jobs == 1:
                ...
                jobs.append(concat_jobs([
                    Job([os.path.abspath(inputs)], [output_tmp], command="ln -s -f " + os.path.abspath(inputs) + " " + output_tmp, samples=[tumor_pair.normal, tumor_pair.tumor]),
                    pipe_jobs([
                        Job([output_tmp], [None],
    ...
    def ensemble_somatic(self):
        ...
        for tumor_pair in self.tumor_pairs.itervalues():
            ...
            #inputs_somatic = [input_mutect2, input_strelka2, input_vardict, input_varscan2, input_samtools]
    ...
    def gatk_variant_annotator_somatic(self):
        ...
        for tumor_pair in self.tumor_pairs.itervalues():
            ...
            if nb_jobs == 1:
                ...
                jobs.append(concat_jobs([
                    Job(command="mkdir -p " + ensemble_directory, removable_files=[output_somatic_variants],
                ...
                for idx, sequences in enumerate(unique_sequences_per_job):
                    output_somatic_variants = os.path.join(ensemble_directory, tumor_pair.name, tumor_pair.name + ".ensemble.somatic.vt.annot." + str(idx) + ".vcf.gz")

                    jobs.append(concat_jobs([
                        Job(command="mkdir -p " + ensemble_directory, removable_files=[output_somatic_variants], samples=[tumor_pair.normal, tumor_pair.tumor]),
                ...
                output_somatic_variants = os.path.join(ensemble_directory, tumor_pair.name, tumor_pair.name + ".ensemble.somatic.vt.annot.others.vcf.gz")
    ...
    def gatk_variant_annotator_germline(self):
        ...
        for tumor_pair in self.tumor_pairs.itervalues():
            input_normal = os.path.join("alignment", tumor_pair.normal.name, tumor_pair.normal.name + ".sorted.dup.recal.bam")
            input_tumor = os.path.join("alignment", tumor_pair.tumor.name, tumor_pair.tumor.name + ".sorted.dup.recal.bam")
            input_germline_variants = os.path.join(ensemble_directory, tumor_pair.name, tumor_pair.name + ".ensemble.germline_loh.vt.vcf.gz")
    
            if nb_jobs == 1:
                output_germline_variants = os.path.join(ensemble_directory, tumor_pair.name, tumor_pair.name + ".ensemble.germline_loh.vt.annot.vcf.gz")
        
                jobs.append(concat_jobs([
                    Job(command="mkdir -p " + ensemble_directory, removable_files=[output_germline_variants], samples=[tumor_pair.normal, tumor_pair.tumor]),
                ...
                unique_sequences_per_job, unique_sequences_per_job_others = split_by_size(self.sequence_dictionary, nb_jobs - 1)
                for idx, sequences in enumerate(unique_sequences_per_job):
                    output_germline_variants = os.path.join(ensemble_directory, tumor_pair.name, tumor_pair.name + ".ensemble.germline_loh.vt.annot." + str(idx) + ".vcf.gz")
                ...
                output_germline_variants = os.path.join(ensemble_directory, tumor_pair.name, tumor_pair.name + ".ensemble.germline_loh.vt.annot.others.vcf.gz")
    ...
    def merge_gatk_variant_annotator_somatic(self):
        ...
        for tumor_pair in self.tumor_pairs.itervalues():
            ...
            if nb_jobs > 1:
                unique_sequences_per_job, unique_sequences_per_job_others = split_by_size(self.sequence_dictionary, nb_jobs - 1)
                vcfs_to_merge = [os.path.join(ensemble_directory, tumor_pair.name, tumor_pair.name + ".ensemble.somatic.vt.annot." + str(idx) +".vcf.gz")
                                  for idx in xrange(len(unique_sequences_per_job))]
                
                vcfs_to_merge.append(os.path.join(ensemble_directory, tumor_pair.name, tumor_pair.name + ".ensemble.somatic.vt.annot.others.vcf.gz"))
                
                jobs.append(concat_jobs([
                    Job(samples=[tumor_pair.normal, tumor_pair.tumor]),
                    pipe_jobs([
                        bcftools.concat(vcfs_to_merge, None),
                        htslib.bgzip_tabix(None, output_somatic),
                    ]),
    ...
    def merge_gatk_variant_annotator_germline(self):
        ...
        for tumor_pair in self.tumor_pairs.itervalues():
            output_germline = os.path.join(ensemble_directory, tumor_pair.name, tumor_pair.name + ".ensemble.germline_loh.vt.annot.vcf.gz")
            if nb_jobs > 1:
                unique_sequences_per_job, unique_sequences_per_job_others = split_by_size(self.sequence_dictionary, nb_jobs - 1)
                vcfs_to_merge = [os.path.join(ensemble_directory, tumor_pair.name, tumor_pair.name + ".ensemble.germline_loh.vt.annot." + str(idx) + ".vcf.gz")
                                 for idx in xrange(len(unique_sequences_per_job))]

                vcfs_to_merge.append(os.path.join(ensemble_directory, tumor_pair.name, tumor_pair.name + ".ensemble.germline_loh.vt.annot.others.vcf.gz"))
        
                jobs.append(concat_jobs([
                    Job(samples=[tumor_pair.normal, tumor_pair.tumor]),
                    pipe_jobs([
                        bcftools.concat(vcfs_to_merge, None),
                        htslib.bgzip_tabix(None, output_germline),
                    ]),
    ...
    def somatic_signature(self):
        ...
        for tumor_pair in self.tumor_pairs.itervalues():
            jobs.append(concat_jobs([
                Job(command="mkdir -p " + somatic_signature_directory + "/tmp_vcf"),
        ...
        jobs.append(concat_jobs([
            Job(input_list, [os.path.join(somatic_signature_directory, "Samplelist.tsv")],
                command="echo -e 'tumor\tpath' > " + os.path.join(somatic_signature_directory,
                                                                  "Samplelist.tsv") + " && cat " + " ".join(
                    input_list) + " >> " + os.path.join(somatic_signature_directory, "Samplelist.tsv"),
                removable_files=input_list),
            tools.r_somatic_signature(os.path.join(somatic_signature_directory, "Samplelist.tsv"),
    ...
    def all_pairs_compute_effects_somatic(self):
        ...
        for tumor_pair in self.tumor_pair.itervalues():
            cancer_pair.write(tumor_pair.normal.name + "\t" + tumor_pair.tumor.name + "\n")
    ...
    def sequenza(self):
        ...
        for tumor_pair in self.tumor_pairs.itervalues():
            ...
            seqz_outputs = [os.path.join(sequenza_directory, "rawSequenza", tumor_pair.name + ".binned.seqz." + sequence['name'] + ".gz") for sequence in self.sequence_dictionary_variant() if sequence['type'] is 'primary']
            ...
            jobs.append(concat_jobs([
                ...
                Job(seqz_outputs, [tmp_output], command="zcat \\\n  " + " \\\n  ".join(seqz_outputs) + " \\\n  | grep -v 'chrom' | grep -v 'MT' | gzip -cf > " + tmp_output, removable_files=[tmp_output]),
    ...
    def sCNAphase(self):
        ...
        for tumor_pair in self.tumor_pairs.itervalues():
            ...
            mkdir_job=Job(command="mkdir -p " + raw_directory, removable_files=[raw_directory], samples=[tumor_pair.normal, tumor_pair.tumor])
            ...
            if nb_jobs == 1:
                jobs.append(concat_jobs([
                    mkdir_job,
                    pipe_jobs([
                        samtools.mpileup([inputNormal], None, config.param('samtools_single', 'mpileup_other_options'), ini_section="samtools_single"),
                        samtools.bcftools_view("-", os.path.join(raw_directory, tumor_pair.normal.name + ".bcf"),
                                                    config.param('samtools_single', 'bcftools_view_options'), ini_section="samtools_single"),
                    ]),
                ...
                jobs.append(concat_jobs([
                    mkdir_job,
                    pipe_jobs([
                        samtools.mpileup([inputTumor], None, config.param('samtools_single', 'mpileup_other_options'), ini_section="samtools_single"),
                        samtools.bcftools_view("-", os.path.join(raw_directory, tumor_pair.tumor.name + ".bcf"),
                                                    config.param('samtools_single', 'bcftools_view_options'), ini_section="samtools_single"),
                    ]),
                ...
                for region in self.generate_approximate_windows(nb_jobs):  # for idx,sequences in enumerate(unique_sequences_per_job):
                    jobs.append(concat_jobs([
                        mkdir_job,
                        pipe_jobs([
                            samtools.mpileup([inputNormal], None, config.param('samtools_single', 'mpileup_other_options'), region, ini_section="samtools_single"),
                            samtools.bcftools_view("-", os.path.join(raw_directory, tumor_pair.normal.name + "." + region + ".bcf"),
                                                        config.param('samtools_single', 'bcftools_view_options'), ini_section="samtools_single"),
                        ]),
                    ...
                    jobs.append(concat_jobs([
                        mkdir_job,
                        pipe_jobs([
                            samtools.mpileup([inputTumor], None, config.param('samtools_single', 'mpileup_other_options'), region, ini_section="samtools_single"),
                            samtools.bcftools_view("-", os.path.join(raw_directory, tumor_pair.tumor.name + "." + region + ".bcf"),
                                                        config.param('samtools_single', 'bcftools_view_options'), ini_section="samtools_single"),
                        ]),
                ...
                jobs.append(concat_jobs([
                    pipe_jobs([
                        samtools.bcftools_cat(inputsNormal, None, ini_section="samtools_single"),
                        samtools.bcftools_view("-", None, ini_section="samtools_single"),
                        Job([None], [None], command="awk -F$'\\t' -v OFS='\\t' '{if ($0 !~ /^#/) gsub(/[KMRYSWBVHDX]/, \"N\", $4) } {print}'"),
                        Job([None], [None], command="awk -F$'\\t' -v OFS='\\t' '{if ($0 !~ /^#/) gsub(/[KMRYSWBVHDX]/, \"N\", $5) } {print}'"),
                        Job([None], [None], command="awk -F$'\\t' -v OFS='\\t' '$1!~/^#/ && $4 == $5 {next} {print}'"),
                    ...
                    pipe_jobs([
                        Job([output_normal_vcf_vt], [None], command="zgrep -Pv '\\tN\\t' " + output_normal_vcf_vt + " | grep -v 'INDEL' | grep -v '\.\/' | grep -v '\/\.' "),
                ...
                jobs.append(concat_jobs([
                    pipe_jobs([
                        samtools.bcftools_cat(inputsTumor, None, ini_section="samtools_single"),
                        samtools.bcftools_view("-", None, ini_section="samtools_single"),
                        Job([None], [None], command="awk -F$'\\t' -v OFS='\\t' '{if ($0 !~ /^#/) gsub(/[KMRYSWBVHDX]/, \"N\", $4) } {print}'"),
                        Job([None], [None], command="awk -F$'\\t' -v OFS='\\t' '{if ($0 !~ /^#/) gsub(/[KMRYSWBVHDX]/, \"N\", $5) } {print}'"),
                        Job([None], [None], command="awk -F$'\\t' -v OFS='\\t' '$1!~/^#/ && $4 == $5 {next} {print}'"),
                    ...
                    pipe_jobs([
                        Job([output_tumor_vcf_vt], [None], command="zgrep -Pv '\\tN\\t' " + output_tumor_vcf_vt + " | grep -v 'INDEL' | grep -v '\.\/' | grep -v '\/\.' "),
    ...
    def delly_sv_annotation(self):
        ...
        for tumor_pair in self.tumor_pairs.itervalues():
            ...
            jobs.append(concat_jobs([
                vawk.paired_somatic(output_flt_vcf, tumor_pair.normal.name, tumor_pair.tumor.name, final_directory + ".delly.somatic.vcf"),
            ...
            jobs.append(concat_jobs([
                vawk.paired_germline(output_flt_vcf, tumor_pair.normal.name, tumor_pair.tumor.name, final_directory + ".delly.germline.vcf"),
    ...
    def lumpy_sv_annotation(self):
        ...
        for tumor_pair in self.tumor_pairs.itervalues():
            ...
            jobs.append(concat_jobs([
                pipe_jobs([
                    vawk.paired_somatic(genotype_vcf, tumor_pair.normal.name, tumor_pair.tumor.name, None),
                    htslib.bgzip(None, somatic_vcf),
                ]),
                pipe_jobs([
                    vawk.paired_germline(genotype_vcf, tumor_pair.normal.name, tumor_pair.tumor.name, None),
    ...
    def wham_call_sv(self):
        ...
        for tumor_pair in self.tumor_pairs.itervalues():
            ...
            mkdir_job = Job(command="mkdir -p " + wham_directory, removable_files=[wham_directory], samples = [tumor_pair.normal, tumor_pair.tumor])
            ...
            jobs.append(concat_jobs([
                ...
                pipe_jobs([
                    ...
                    vt.sort("-", os.path.join(pair_directory, tumor_pair.name + ".wham.sorted.vcf"), "-m full"),
                ]),
                pipe_jobs([
    ...
    def cnvkit_batch(self):
        ...
        for tumor_pair in self.tumor_pairs.itervalues():
            ...
            vardict_vcf = os.path.join("pairedVariants", tumor_pair.name, tumor_pair.name + ".vardict.germline_loh.vt.vcf.gz")
    ...
    def svaba_assemble(self):
        ...
        for tumor_pair in self.tumor_pairs.itervalues():
            ...
            mkdir_job = Job(command="mkdir -p " + svaba_directory, removable_files=[svaba_directory], samples = [tumor_pair.normal, tumor_pair.tumor])
