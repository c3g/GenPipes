...
class PacBioAssembly(common.MUGQICPipeline):
    ...
    def smrtanalysis_filtering(self):
        ...
        jobs = []
        jobs.append(Job([self.args.readsets.name], [config.param('smrtanalysis_filtering', 'celera_settings'), config.param('smrtanalysis_filtering', 'filtering_settings')], command="cp -a -f " + os.path.join(os.path.dirname(os.path.abspath(sys.argv[0])), "protocols") + " .", name="smrtanalysis_filtering.config"))
