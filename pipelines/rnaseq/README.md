...
downstream analysis to determine genes and transcripts differential expression. Two options are available
for transcript analysis: [Stringtie](http://ccb.jhu.edu/software/stringtie) as well as 
the [Cufflinks](http://cufflinks.cbcb.umd.edu/) suite. Whereas 
...
The differential gene analysis is followed by a Gene Ontology (GO) enrichment analysis using
the [goseq approach](http://bioconductor.org/packages/release/bioc/html/goseq.html).
This method is based on the use of non-native GO terms (see details in the section 5 of
[the corresponding vignette](http://bioconductor.org/packages/release/bioc/vignettes/goseq/inst/doc/goseq.pdf)).

Finally, a summary html report is generated by the pipeline at the end of the analysis.
...
usage: rnaseq.py [-h] [--help] [-c CONFIG [CONFIG ...]] [-s STEPS]
                 ...
                 [-d DESIGN] [-t {stringtie,cufflinks}] [-r READSETS] [-v]
...
optional arguments:
  ...
  -t {stringtie,cufflinks}, --type {stringtie,cufflinks}
                        Type of pipeline to use for transcript analysis
...
stringtie
---------
Run [stringtie](http://ccb.jhu.edu/software/stringtie) to assemble transcripts using the alignment data.

stringtie_merge
---------------
Merge individual assemblies to form a single assembly for all samples. 

stringtie_abund
---------------
Use merged transcript assembly to estimate abundance of transcripts in each sample. 

ballgown
--------
Use [ballgown](http://bioconductor.org/packages/release/bioc/html/ballgown.html) to calculate differential transcript expression levels and test them for significant differences. 
