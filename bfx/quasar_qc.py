...
def restructure_matrix(input_file_path, output_file_path,  output_dir, bin, temp_dir):
    ...
    return Job(
     ...
     [input_file_path],
        ...
        command="""\
mkdir -p {output_dir}/{temp_dir} &&
awk -v OFS="\\t" '{{if(NR!=1) {{n=split($1, a, "-"); sub(/-[0-9]+$/, "", $1); print $1":"a[n]"-"{bin}+a[n],$0 }} else {{print "V1",$0}} }}' {input_file} | \\
cut -f2 --complement | \\
...
hifive quasar -p {output_project_file} -r {res} -d {coverage} -o {report_file} {quasar_file} --seed 12345""".format(
...
i=1 &&
for f in {out_dir}/{temp_dir}/*_{quasr_prefix}_{res}_{enzyme}_report.txt; do
    awk -v OFS="\\t" '{{if($1=="Resolution") {{print $0}} else {{print FILENAME,$0 }} }}' $f | \\
    tr -d " \\r" | \\
    ...
    cut -f2 --complement >> {output_file}; i=$(($i+1));
done""".format(
            input_files=input_files,
            output_file=output_file,
            res=res,
