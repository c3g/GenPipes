...
for (item in c("genes","isoforms")){    
    for (contrast in contrasts){
        ...
        if (!inherits(subset.dge,'try-error') && nrow(subset.dge) > 0){
            print(kable(subset.dge, row.names=F, caption=paste("Differentially Expressed ",item," for contrast ", contrast, 
        ...
        }else  print(paste("No differentially expressed ",item," results are available for this design after filtering.",sep=""))    
        ...
        if (!inherits(subset.goseq,'try-error') && nrow(subset.goseq) > 0){
            print(kable(subset.goseq, row.names=F, caption=paste("GO Results of the Differentially Expressed ",item," for contrast ", contrast, 
        ...
        } else print(paste("No FDR adjusted GO enrichment was significant (p-value too high) based on the differentially expressed ",item, " results for this design.",sep=""))            
    }
}
```