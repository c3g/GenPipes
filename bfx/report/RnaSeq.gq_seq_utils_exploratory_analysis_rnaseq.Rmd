...
log2(FPKM) (fragments per kilobase of exon per million fragments mapped).The main goals of expression level EDA is to  
detect the possible presence of outlying samples, potential mislabeling, explore the homogeneity of biological replicates and appreciate 
the global effects of the different experimental variables. EDA plots are therefore essential to insure clean data is sent to differential analysis.

First, we analyze how different samples are connected to each other using 
hierarchical clustering on the log2(CPM) gene expression levels, using 
the correlation distance and the Ward method.
