...
all_readsets_abundance.transcripts=read.delim(file.path(report_dir,"kallisto/All_readsets/all_readsets.abundance_transcripts.csv"), as.is=T)
all_readsets_abundance.genes=read.delim(file.path(report_dir,"kallisto/All_readsets/all_readsets.abundance_genes.csv"), as.is=T)

all_readsets_triming_metrics=read.delim(file.path(report_dir,"metrics/trimReadsetTable.tsv"), as.is=T)
...
for (i in seq(length(readset_names))){
    ...
    mat[i,"Raw Reads #"]=all_readsets_triming_metrics[readset, "Paired Reads #"]
    mat[i,"Surviving Reads"]=all_readsets_triming_metrics[readset,"Surviving Paired Reads #"]
    ...
    mat[i, "Transcripts Reads #"]=sum(transcript_count)
    ...
    mat[i,"Genes Reads #"]=sum(gene_count)
    mat[i,11]=signif(mat[i,"Genes Reads #"]/mat[i,"Raw Reads #"],3)*100
}
#mat=t(apply(mat, 1, function(x){prettyNum(x,big.mark=",",scientific=FALSE)}))
...
Kallisto is ran on fastq file, producing raw count estimates at the transcript level. Then, the R package tximport is used to generate the gene count. You can retrieve the transcripts to genes correspondance file [here](Mus_musculus.GRCm38.Ensembl83.tx2gene).
